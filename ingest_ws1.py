...
db_file = os.path.join(base_dir, 'ws1_leads.duckdb')

print("Reading WS1.xlsx ...")
df = pd.read_excel(excel_file, sheet_name='Sheet1')
print(f"  → {len(df):,} rows, {len(df.columns)} columns")

# Clean column names for easier SQL querying (same logic as ingest.py)
def clean_col(name):
    name = str(name).strip()
